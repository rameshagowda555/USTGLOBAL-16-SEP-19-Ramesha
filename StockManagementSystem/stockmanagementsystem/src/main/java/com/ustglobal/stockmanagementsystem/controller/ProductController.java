...
public class ProductController {
	...
	@GetMapping(path = "/order-product/{quantity}/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
	public UserResponse orderProduct(@PathVariable("quantity")int quantity,@PathVariable("id")int id) {
		UserResponse response = new UserResponse();
		Products_Info info = new Products_Info();
		int total_price = (int) (info.getPrice()*quantity);
		int total_price_gst= total_price+10;
		Order_info order = new Order_info();
		order.setTotal_price(total_price);
		order.setTotal_price_with_gst(total_price_with_gst);
		List<Products_Info> bean = service.orderProduct(order, quantity, id);
		if (!bean.isEmpty()) {
			response.setStatusCode(201);
			response.setMessage("Success");
			response.setDescription("Products Are Found in the Store");
